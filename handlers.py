...
async def save_response(user_data):
    try:
...
async def general_health(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    filename = await save_response(context.user_data)  # Сохраняем ответы в файл

    if filename:
        await send_responses_to_admins(context.application, filename)
